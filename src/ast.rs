...
fn fmt_tuple<T: Display, F>(f: &mut Formatter<'_>, v: &Vec<T>, fmt: F) -> Result
		where F: Fn(&mut Formatter<'_>, &T) -> Result {
	match v.len() {
		0 => write!(f, "()"),
		1 => {
			write!(f, "(")?;
...
fn fmt_list<T, F>(f: &mut Formatter<'_>, v: &Vec<T>, left: &str, right: &str, pat: &str, fmt: F) -> Result
		where F: Fn(&mut Formatter<'_>, &T) -> Result {
	write!(f, "{}", left)?;
	let mut it =v.iter();
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
			...
			Expr::UnaryOp(op, expr) => match expr {
				box Expr::BinOp(_, _, _) | box Expr::Apply(_, _) => write!(f, "{}({})", op.to_str(), expr),
			...
			Expr::List(exprs) => fmt_list(f, exprs, "[", "]", ", ", |f, expr|{write!(f, "{}", expr)}),
			Expr::Nil(expr) => write!(f, "nil ({})", expr),
			Expr::Head(expr) => write!(f, "head ({})", expr),
			Expr::Tail(expr) => write!(f, "tail ({})", expr),
			Expr::IfThenElse(cond, expr0, expr1) => write!(f, "(if {} then {} else {})", cond, expr0, expr1),
